...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderList(filter = '') {
        ...
        instructivos.filter(i => (i.titulo||'').toLowerCase().includes(filter.toLowerCase())).forEach(i => {
            ...
            div.innerHTML = `
                <div style="flex:1; min-width:0;" onclick="(function(){})()">
                    <h4 style="margin:0 0 2px; font-size:13px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${i.titulo || 'Sin Título'}</h4>
                    <p style="margin:0; font-size:11px; opacity:.75;">${i.evento || '---'}</p>
                </div>
                <button class="btn-delete-item" title="Eliminar instructivo" onclick="event.stopPropagation(); deleteItem('${i.id}')">🗑</button>
            `;
